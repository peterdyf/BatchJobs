...
@ContextConfiguration(classes = { TestConfigs.class, BillPaymentTemplateDeletion.class, SpAuditLogInsert.class, TestConfigs4Ftp.class })
@ActiveProfiles({ ProfileNames.TEST, ProfileNames.CONFIG_DEV })
public class TestBillPaymentTemplateDeletion {
	...
	private static final String CSV_FILE_NAME = BillPaymentTemplateDeletion.FTP_FOLDER + File.separator + "BillPaymentTemplateDeletion.csv";

	@Before
	public void setUp() {
		fileUtil.clean(CSV_FILE_NAME);

		int duration = sysParam.getInt("BP_TMP_DURATION");
	...
	private void insert(String pbUserId, String templateName, int duration) {
		...
		//@formatter:off
		sql.insertInto(DSL.table("TBL_PB_BILL_PYMT_TMP"))
			...
			.set(DSL.field("IB_MCHNT_ID", String.class), "D3C5D9AB57153901E03400306EC38365")
			.execute();
		//@formatter:on
	}

	private BigDecimal select(String templateName) {
		return sql.select().from("TBL_PB_BILL_PYMT_TMP").where("TEMPLATE_NM = ?", templateName).fetchOne(DSL.field("IS_DEL", BigDecimal.class));
	}
	...
	private int delete(String templateName) {
		return sql.delete(DSL.table("TBL_PB_BILL_PYMT_TMP")).where("TEMPLATE_NM = ?", templateName).execute();
	}
